#!/bin/bash
# 安全版:完整修复Spine 4.0从cocos2d到axmol的API差异(基于官方提交历史)

SPINE_DIR="./spine/src/spine"

echo "========================================="
echo "开始修复Spine 4.0 API (安全版本)"
echo "========================================="
echo ""

# 1. 替换 cocos2d.h → axmol.h
echo "[1/14] 替换 #include cocos2d.h → axmol.h"
find "$SPINE_DIR" -type f \( -name "*.h" -o -name "*.cpp" \) -exec sed -i 's/#include "cocos2d\.h"/#include "axmol.h"/g' {} \;

# 2. 替换 USING_NS_CC → using namespace ax
echo "[2/14] 替换 USING_NS_CC → using namespace ax"
...
# 3. 替换 CC_DEPRECATED_ATTRIBUTE → AX_DEPRECATED(2.1)
echo "[3/14] 替换 CC_DEPRECATED_ATTRIBUTE → AX_DEPRECATED(2.1)"
...
# 4. 删除 CC_CONSTRUCTOR_ACCESS : 和 AX_CONSTRUCTOR_ACCESS :
echo "[4/14] 删除 CC_CONSTRUCTOR_ACCESS : 和 AX_CONSTRUCTOR_ACCESS :"
...
echo "[8/14] 替换类名 Cocos2d前缀 → Axmol"
...
echo "[11/14] 删除 drawNode->setLineWidth() 调用"
find "$SPINE_DIR" -type f -name "*.cpp" -exec sed -i '/drawNode->setLineWidth/d' {} \;

# 12. 修复 drawPoly 调用 (添加lineWidth参数) - 更精确的匹配
echo "[12/14] 修复 drawPoly 调用添加 lineWidth 参数"
find "$SPINE_DIR" -type f -name "*.cpp" -exec sed -i 's/drawPoly(\([^)]*\), true, Color4B::\([A-Z]*\))/drawPoly(\1, true, Color4B::\2, 2.0f)/g' {} \;

# 13. 修复 drawLine 调用 (添加lineWidth参数)
echo "[13/14] 修复 drawLine 调用添加 lineWidth 参数"
find "$SPINE_DIR" -type f -name "*.cpp" -exec sed -i 's/drawLine(\([^,]*\), \([^,]*\), Color4B::\([A-Z]*\))/drawLine(\1, \2, Color4B::\3, 2.0f)/g' {} \;

# 14. 清理可能的双重ax::前缀
echo "[14/14] 清理双重 ax:: 前缀"
...
echo "- 宏: CC_DEPRECATED_ATTRIBUTE → AX_DEPRECATED(2.1)"
echo "- 类名: Cocos2d → Axmol"
echo "- DrawNode: Color4F → Color4B, 添加 lineWidth 参数"
echo "- 删除: CC_CONSTRUCTOR_ACCESS, setLineWidth()"
echo ""
echo "注意事项:"
echo "- 已避免误替换 Texture2D、Data、FileUtils 等类型"
echo "- 这些类型在 using namespace ax 后会自动解析"
echo "- 如果有编译错误,请手动添加 ax:: 前缀"
echo ""
